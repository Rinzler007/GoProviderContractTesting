...
class ProviderPactTest(unittest.TestCase):
 ...
 def verify_contract(self, consumer_query_param, auth_required=True):
    """Reusable method to verify contract based on query parameter and authorization need."""
 ...
 def test_verify_consumer_fullserve(self):
    ...
    self.assertEqual(self.verify_contract("?consumer=fullserve"), 0, "Provider verification failed for 'fullserve' consumer")

 def test_verify_consumer_veripark(self):
    """Verifies the contract for the 'veripark' consumer."""
    self.assertEqual(self.verify_contract("?consumer=veripark"), 0, "Provider verification failed for 'veripark' consumer")

 def test_verify_consumer_salesforce(self):
    """Verifies the contract for the 'salesforce' consumer."""
    self.assertEqual(self.verify_contract("?consumer=salesforce"), 0, "Provider verification failed for 'salesforce' consumer")

 def test_verify_consumer_invalid(self):
    """Verifies the contract for an 'invalid' consumer."""
    self.assertEqual(self.verify_contract("?consumer=invalid"), 0, "Provider verification failed for 'invalid' consumer")

 def test_verify_consumer_empty(self):
    """Verifies the contract for an empty consumer query."""
    self.assertEqual(self.verify_contract("?consumer="), 0, "Provider verification failed for 'empty consumer'")

 def test_verify_missing_authorization(self):
    """Verifies the contract when Authorization header is missing."""
    self.assertEqual(self.verify_contract("?consumer=fullserve", auth_required=False), 0, "Provider verification failed for missing Authorization")
